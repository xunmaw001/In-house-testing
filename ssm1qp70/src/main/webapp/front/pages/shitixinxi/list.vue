	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.ceshimingcheng" type="text" placeholder="测试名称" ></input>
			</view>
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.nandudengji" type="text" placeholder="难度等级" ></input>
			</view>
			<view class="action">
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
								                				                				                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.ceshimingcheng}}</view>
					...
					<text v-if="isAuth('shitixinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('shitixinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"8rpx","color":"#fff","borderWidth":"2rpx","width":"160rpx","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('shitixinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.ceshimingcheng="";
				this.searchForm.nandudengji="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(indexQueryCondition) {
					params['nandudengji'] = '%' + indexQueryCondition + '%';
					uni.removeStorageSync('indexQueryCondition');
				}

				let res = await this.$api.list(`shitixinxi`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shitixinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['ceshimingcheng'] = '%' + this.searchForm.ceshimingcheng + '%'
				}	
																								if(this.searchForm.nandudengji){
					searchForm['nandudengji'] = '%' + this.searchForm.nandudengji + '%'
				}	
																																																				let res = await this.$api.list(`shitixinxi`, searchForm);
				// 如果是第一页数据置空
