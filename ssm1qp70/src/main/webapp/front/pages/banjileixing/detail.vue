<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>分类：</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','banjileixing');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','banjileixing');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('banjileixing', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussbanjileixing/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('banjileixing', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('banjileixing', this.detail);
				this.$utils.msg('审核成功');
